/* file-dict.c generated by valac 0.14.0, the Vala compiler
...
static void skk_file_dict_load (SkkFileDict* self, GError** error) {
	...
	offset = (glong) 0;
	_tmp1_ = skk_file_dict_read_until (self, &offset, ";; okuri-ari entries.\n");
	if (!_tmp1_) {
		GError* _tmp2_;
		_tmp2_ = g_error_new_literal (SKK_SKK_DICT_ERROR, SKK_SKK_DICT_ERROR_MALFORMED_INPUT, "no okuri-ari boundary");
	...
	_tmp3_ = offset;
	self->priv->okuri_ari_offset = _tmp3_;
	_tmp4_ = skk_file_dict_read_until (self, &offset, ";; okuri-nasi entries.\n");
	if (!_tmp4_) {
		GError* _tmp5_;
		_tmp5_ = g_error_new_literal (SKK_SKK_DICT_ERROR, SKK_SKK_DICT_ERROR_MALFORMED_INPUT, "no okuri-nasi boundary");
	...
	_tmp6_ = offset;
	self->priv->okuri_nasi_offset = _tmp6_;
}
	...
	if (g_strcmp0 (_tmp2_, _tmp3_) != 0) {
		{
			...
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == SKK_SKK_DICT_ERROR) {
					goto __catch1_skk_skk_dict_error;
				}
		...
		__catch1_skk_skk_dict_error:
		{
			...
			g_warning ("file-dict.vala:114: error loading file dictionary %s %s", _tmp8_, _tmp10_);
			_g_free0 (_tmp8_);
			_g_error_free0 (e);
		}
		__finally1:
...
static gboolean skk_file_dict_search_pos (SkkFileDict* self, const gchar* midasi, glong start_offset, glong end_offset, GCompareFunc cmp, glong* pos, gchar** line, gint direction) {
	...
	while (TRUE) {
		...
		if (_tmp12_ < 1) {
			const gchar* _tmp13_;
			_tmp13_ = _line;
			g_warning ("file-dict.vala:134: corrupted dictionary entry: %s", _tmp13_);
			_g_free0 (_line);
			break;
		}
		_tmp14_ = cmp;
...
static SkkCandidate** skk_file_dict_real_lookup (SkkDict* base, const gchar* midasi, gboolean okuri, int* result_length1) {
	...
	__catch2_g_error:
	{
		...
		g_warning ("file-dict.vala:176: can't encode %s: %s", _tmp16_, _tmp18_);
	...
	if (_tmp30_) {
		...
		if (_tmp33_ > 0) {
			...
			__catch3_g_error:
			{
				...
				g_warning ("file-dict.vala:195: can't decode line %s: %s", _tmp45_, _tmp47_);
...
static gchar** skk_file_dict_real_complete (SkkDict* base, const gchar* midasi, int* result_length1) {
	...
	__catch4_g_error:
	{
		...
		g_warning ("file-dict.vala:228: can't decode %s: %s", _tmp14_, _tmp16_);
		_tmp17_ = completion;
	...
	if (_tmp26_) {
		...
		_line = _tmp29_;
		{
			...
			while (TRUE) {
				...
				if (_tmp42_ < 0) {
					const gchar* _tmp43_;
					_tmp43_ = line;
					g_warning ("file-dict.vala:248: corrupted dictionary entry: %s", _tmp43_);
				} else {
					const gchar* _tmp44_;
					gint _tmp45_;
					gchar* _tmp46_ = NULL;
					...
					if (g_strcmp0 (_tmp47_, _tmp48_) != 0) {
						...
						__catch5_g_error:
						{
							...
							g_warning ("file-dict.vala:258: can't decode line %s: %s", _tmp54_, _tmp56_);
							_g_error_free0 (e);
						}
						__finally5:
						if (_inner_error_ != NULL) {
		...
		while (TRUE) {
			...
			if (_tmp70_ < 0) {
				const gchar* _tmp71_;
				_tmp71_ = line;
				g_warning ("file-dict.vala:273: corrupted dictionary entry: %s", _tmp71_);
			} else {
				const gchar* _tmp72_;
				gint _tmp73_;
				gchar* _tmp74_ = NULL;
				...
				if (g_strcmp0 (_tmp75_, _tmp76_) != 0) {
					...
					__catch6_g_error:
					{
						...
						g_warning ("file-dict.vala:283: can't decode line %s: %s", _tmp82_, _tmp84_);
						_g_error_free0 (e);
					}
					__finally6:
					if (_inner_error_ != NULL) {
