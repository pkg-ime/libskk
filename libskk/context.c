/* context.c generated by valac 0.14.1, the Vala compiler
...
static void __lambda4_ (SkkContext* self, SkkCandidate* candidate) {
	...
	if (_tmp1_) {
		...
		__catch36_g_error:
		{
			...
			g_warning ("context.vala:243: error saving dictionaries %s", _tmp3_);
			_g_error_free0 (e);
		}
		__finally36:
		if (_inner_error_ != NULL) {
	...
	if (_tmp2_) {
		...
		if (_tmp8_) {
			...
			__catch37_g_error:
			{
				...
				g_warning ("context.vala:370: error saving dictionaries %s", _tmp10_);
				_g_error_free0 (e);
			}
			__finally37:
			if (_inner_error_ != NULL) {
...
gboolean skk_context_process_key_events (SkkContext* self, const gchar* keyseq) {
	...
	while (TRUE) {
		...
		switch (_tmp8_) {
			...
			case '(':
			{
				...
				_tmp9_ = complex;
				if (_tmp9_) {
					g_warning ("context.vala:435: bare '(' is not allowed in complex keyseq");
					result = FALSE;
			...
			case ')':
			{
				...
				_tmp12_ = complex;
				if (!_tmp12_) {
					g_warning ("context.vala:443: bare ')' is not allowed in simple keyseq");
					result = FALSE;
					_g_string_free0 (builder);
					_g_object_unref0 (keys);
					return result;
	...
	_tmp30_ = complex;
	if (_tmp30_) {
		g_warning ("context.vala:466: premature end of key events");
		result = FALSE;
		_g_string_free0 (builder);
		_g_object_unref0 (keys);
		return result;
	...
	retval = FALSE;
	{
		...
		while (TRUE) {
			...
			}
			{
				...
				SkkKeyEvent* _tmp57_;
				_tmp55_ = key;
				_tmp56_ = skk_key_event_new_from_string (_tmp55_, &_inner_error_);
				_tmp57_ = _tmp56_;
				if (_inner_error_ != NULL) {
					if (_inner_error_->domain == SKK_KEY_EVENT_FORMAT_ERROR) {
						goto __catch38_skk_key_event_format_error;
					}
					_g_object_unref0 (ev);
					_g_free0 (key);
			...
			goto __finally38;
			__catch38_skk_key_event_format_error:
			{
				GError* e = NULL;
				const gchar* _tmp58_;
				GError* _tmp59_;
				...
				g_warning ("context.vala:488: can't get key event from string %s: %s", _tmp58_, _tmp60_);
				result = FALSE;
				_g_error_free0 (e);
				_g_object_unref0 (ev);
				_g_free0 (key);
			...
			_tmp62_ = ev;
			_tmp63_ = skk_context_process_key_event (self, _tmp62_);
			if (_tmp63_) {
				gboolean _tmp64_;
				_tmp64_ = retval;
				_tmp61_ = !_tmp64_;
...
static gchar* skk_context_retrieve_output (SkkContext* self, gboolean clear) {
	...
	g_return_val_if_fail (self != NULL, NULL);
	_tmp0_ = self->priv->state_stack;
	_tmp1_ = gee_abstract_list_last ((GeeAbstractList*) _tmp0_);
	...
	_tmp6_ = handler;
	_tmp7_ = state;
	_tmp8_ = skk_state_handler_get_output (_tmp6_, _tmp7_);
	output = _tmp8_;
	_tmp9_ = clear;
	if (_tmp9_) {
		SkkState* _tmp10_;
		...
		g_string_erase (_tmp11_, (gssize) 0, (gssize) (-1));
	}
	result = output;
	_g_object_unref0 (handler);
	_g_object_unref0 (state);
...
static void skk_context_update_preedit (SkkContext* self) {
	...
	while (TRUE) {
		...
		GString* _tmp26_;
		_tmp4_ = iter;
		_tmp5_ = gee_bidir_iterator_has_previous ((GeeBidirIterator*) _tmp4_);
		if (!_tmp5_) {
			break;
		}
		_tmp6_ = iter;
		_tmp7_ = gee_iterator_get ((GeeIterator*) _tmp6_);
		state = (SkkState*) _tmp7_;
		_tmp8_ = self->priv->handlers;
		_tmp9_ = state;
		_tmp10_ = _tmp9_->handler_type;
		_tmp11_ = gee_map_get (_tmp8_, GINT_TO_POINTER (_tmp10_));
		handler = (SkkStateHandler*) _tmp11_;
		_tmp12_ = iter;
		_tmp13_ = gee_iterator_has_next ((GeeIterator*) _tmp12_);
		if (_tmp13_) {
			GString* _tmp14_;
			SkkStateHandler* _tmp15_;
			SkkState* _tmp16_;
			gchar* _tmp17_ = NULL;
			gchar* _tmp18_;
			_tmp14_ = builder;
			_tmp15_ = handler;
			_tmp16_ = state;
			_tmp17_ = skk_state_handler_get_output (_tmp15_, _tmp16_);
			_tmp18_ = _tmp17_;
			g_string_append (_tmp14_, _tmp18_);
			_g_free0 (_tmp18_);
		}
		_tmp19_ = iter;
		gee_bidir_iterator_previous ((GeeBidirIterator*) _tmp19_);
		_tmp20_ = iter;
		_tmp21_ = gee_iterator_get ((GeeIterator*) _tmp20_);
		_g_object_unref0 (state);
		state = (SkkState*) _tmp21_;
		_tmp22_ = builder;
		g_string_append (_tmp22_, "▼");
		_tmp23_ = builder;
		_tmp24_ = state;
		_tmp25_ = _tmp24_->midasi;
	...
	_tmp27_ = self->priv->state_stack;
	_tmp28_ = gee_deque_peek_head ((GeeDeque*) _tmp27_);
	state = (SkkState*) _tmp28_;
	_tmp29_ = self->priv->handlers;
	_tmp30_ = state;
	_tmp31_ = _tmp30_->handler_type;
	_tmp32_ = gee_map_get (_tmp29_, GINT_TO_POINTER (_tmp31_));
	handler = (SkkStateHandler*) _tmp32_;
	_tmp33_ = skk_context_dict_edit_level (self);
	if (_tmp33_ > ((guint) 0)) {
		GString* _tmp34_;
		SkkStateHandler* _tmp35_;
		SkkState* _tmp36_;
		gchar* _tmp37_ = NULL;
		gchar* _tmp38_;
		_tmp34_ = builder;
		_tmp35_ = handler;
		_tmp36_ = state;
		_tmp37_ = skk_state_handler_get_output (_tmp35_, _tmp36_);
		_tmp38_ = _tmp37_;
	...
	_tmp43_ = handler;
	_tmp44_ = state;
	_tmp47_ = skk_state_handler_get_preedit (_tmp43_, _tmp44_, &_tmp45_, &_tmp46_);
	offset = _tmp45_;
	nchars = _tmp46_;
	_tmp48_ = _tmp47_;
	...
	_tmp49_ = offset;
	_tmp50_ = start;
	offset = _tmp49_ + _tmp50_;
	_tmp51_ = skk_context_dict_edit_level (self);
	level = _tmp51_;
	{
		gint i;
		i = 0;
	...
	if (g_strcmp0 (_tmp58_, _tmp60_) != 0) {
		...
		_tmp61_ = builder;
		_tmp62_ = _tmp61_->str;
		skk_context_set_preedit (self, _tmp62_);
		changed = TRUE;
	}
	_tmp64_ = self->priv->preedit_underline_offset;
	_tmp65_ = offset;
	if (_tmp64_ != _tmp65_) {
		_tmp63_ = TRUE;
	} else {
		guint _tmp66_;
		guint _tmp67_;
		_tmp66_ = self->priv->preedit_underline_nchars;
		_tmp67_ = nchars;
		_tmp63_ = _tmp66_ != _tmp67_;
	}
	_tmp68_ = _tmp63_;
	if (_tmp68_) {
		guint _tmp69_;
		guint _tmp70_;
		_tmp69_ = offset;
		self->priv->preedit_underline_offset = _tmp69_;
		_tmp70_ = nchars;
		self->priv->preedit_underline_nchars = _tmp70_;
		changed = TRUE;
	}
	_tmp71_ = changed;
	if (_tmp71_) {
